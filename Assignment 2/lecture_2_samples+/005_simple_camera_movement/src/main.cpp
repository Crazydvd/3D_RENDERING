...
int main () {
    ...
    while (window.isOpen()) {
        ...
        cameraMatrix = glm::translate(cameraMatrix, glm::vec3(0, 0, 5*cos(clock()/100.f)));
        cameraMatrix = glm::rotate(cameraMatrix, glm::radians(clock()/10.0f), glm::vec3(0, 0, 1));
        ...
        for (int i = 0; i < 100; i++) {
            ...
            glUniformMatrix4fv (glGetUniformLocation (programID, "mvpMatrix"), 1, GL_FALSE, glm::value_ptr(vp*modelMatrix));
