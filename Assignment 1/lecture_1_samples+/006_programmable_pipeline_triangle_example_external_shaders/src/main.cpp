...
int main()
{
	...
	while (window.isOpen()) {
		...
		GLint hMultiplierIndex = glGetUniformLocation(programID, "horizontalMultiplier");
		int hMultiplier = 8;
		glUniform1i(hMultiplierIndex, hMultiplier);

		GLint vMultiplierIndex = glGetUniformLocation(programID, "verticalMultiplier");
		int vMultiplier = 8;
		glUniform1i(vMultiplierIndex, vMultiplier);

		GLint color1Index = glGetUniformLocation(programID, "color1");
		glUniform4f(color1Index, 1, 1, 1, 1);

		GLint color2Index = glGetUniformLocation(programID, "color2");
		glUniform4f(color2Index, 0, 0, 0, 1);

		GLint mousePosIndex = glGetUniformLocation(programID, "mousePosition");
		sf::Vector2i mousePos = sf::Mouse::getPosition(window);
		glUniform2f(mousePosIndex, mousePos.x, window.getSize().y - mousePos.y);
